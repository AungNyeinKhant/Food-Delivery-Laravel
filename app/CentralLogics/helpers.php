    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en')
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                //food_open
                if($item->available_time_starts < $rn && $item->available_time_ends > $rn){
                    $item['food_open'] = 1;
                }else{
                    $item['food_open'] = 0;
                }
                ...
                $item['restaurant_active'] = (int) $item->restaurant->active;
                //ANK upgrade package(12.3.2024)
...
// dd($currency);
    ...
    public static function calculate_addon_price($addons, $add_on_qtys)
    {
        ...
        if ($addons) {
            foreach ($addons as $key2 => $addon) {
                ...
                $data[] = ['id' => $addon->id, 'name' => $addon->name, 'image'=> $addon->image, 'price' => $addon->price, 'quantity' => $add_on_qty];
    ...
    public static function subscription_plan_chosen($restaurant_id ,$package_id, $payment_method  ,$discount,$reference=null ,$type=null){
        ...
        try {
            ...
            //ANK update package(12.3.2024)
            $restaurant_subscription->delivery=$package->delivery;
            $restaurant_subscription->take_away=$package->take_away;
            $restaurant_subscription->dine_in=$package->dine_in;
            ...
            //ANK update package(12.3.2024)
            if($restaurant_subscription->delivery == 1 && $package->delivery == 1){
                $restaurant->delivery  = 1;
            }else{
                $restaurant->delivery  = 0;
            }

            if($restaurant_subscription->take_away == 1 && $package->take_away == 1){
                $restaurant->take_away  = 1;
            }else{
                $restaurant->take_away  = 0;
            }

            if($restaurant_subscription->dine_in == 1 && $package->dine_in == 1){
                $restaurant->dine_in  = 1;
            }else{
                $restaurant->dine_in  = 0;
            }
            ...
            $subscription_transaction->package_details=[
                ...
                'takeaway'=>$package->takeaway,
                'dine_in'=>$package->dine_in,
            ];
