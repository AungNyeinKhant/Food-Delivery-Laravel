...
class ProductLogic
{
    ...
    {
        $paginator = Food::active()->restaurantOpen()->type($type);
        if($category_id != 0)
        {
    ...
    public static function get_related_products($product_id)
    {
        ...
        return Food::active()->restaurantOpen()
    ...
    {
        $key = explode(' ', $name);
        $paginator = Food::active()->restaurantOpen()->whereHas('restaurant', function($q)use($zone_id){
            $q->where(function($q1){
                $q1->where('delivery',1)
                ->orWhere('take_away',1);
            })
   ...
   public static function popular_products($zone_id, $limit = null, $offset = null, $type = 'all')
{
    ...
    $paginator = Food::active()->restaurantOpen()
        ->type($type)
    ...
    public static function most_reviewed_products($zone_id, $limit = null, $offset = null, $type='all')
    {
        ...
        $paginator = Food::active()->restaurantOpen()->type($type)->whereHas('restaurant', function($q)use($zone_id){
            $q->where(function($q1){
                $q1->where('delivery',1)
                ->orWhere('take_away',1);
            })
    ...
    public static function recommended_products($zone_id,$restaurant_id,$limit = null, $offset = null, $type='all',$name=null)
    {
        ...
        if($limit != null && $offset != null)
        {
            ...
            $paginator = Food::where('restaurant_id', $restaurant_id)->restaurantOpen()->active()->type($type)->whereHas('restaurant', function($q)use($zone_id){
                $q->where(function($q1){
                    $q1->where('delivery',1)
                    ->orWhere('take_away',1);
                })
