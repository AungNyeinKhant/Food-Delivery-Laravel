...
class ProductController extends Controller
{
    ...
    public function discount(Request $request){
        ...
        $foods = Food::inRandomOrder()->active()->restaurantOpen()
    ...
    $foods = Food::inRandomOrder()->active()->skip($offset)->restaurantOpen()
    ->whereHas('restaurant', function($q)use($zone_id){
                    $q->where(function($q1){
                        $q1->where('delivery',1)
                        ->orWhere('take_away',1);
    ...
    // return response()->json($foodtotal, 200);
    ...
    public function get_searched_products(Request $request)
    {
        ...
        $offset = $request['offset']??1;
        $type = $request->query('type', 'all');
        $products = Food::active()->type($type)->restaurantOpen()
            ->whereHas('restaurant', function($q)use($zone_id){
                $q->where(function($q1){
                    $q1->where('delivery',1)
                    ->orWhere('take_away',1);
    ...
    public function food_or_restaurant_search(Request $request){
        ...
        $foods = Food::active()->restaurantOpen()
        ->whereHas('restaurant', function($q)use($zone_id){
            $q->where(function($q1){
                $q1->where('delivery',1)
                ->orWhere('take_away',1);
